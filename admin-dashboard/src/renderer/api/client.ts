...
async function request<T>(
    ...
    if (!res.ok) {
        const message =
            (body as ApiError).error ?? (body as ApiError).message ?? res.statusText;
...
export async function getAdminStats(
    orgId?: string
): Promise<{ data: AdminStats } | { error: string; status: number }> {
...
export async function getLeads(
    orgId?: string
): Promise<{ data: { leads: Lead[] } } | { error: string; status: number }> {
